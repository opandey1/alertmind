...
**Post-v1 progress.** The `assistant-svc` and `socanalyst` identities named above now exist with least-privilege Indexer roles, and their read/write boundary has been verified against the live cluster — see `evidence/rbac/`. The restricted host-only SSH forward that keeps the Indexer bound to loopback also has owner-executed, independently reviewed live evidence, merged through PR #16. The transport rollback/revocation drill remains pending, and the ingestion path itself remains unimplemented: there is no OIDC/application authentication or authorization, no constrained reader module and no live-alert UI, and no application code reads from Wazuh. SSH public-key authentication and the Indexer's own basic-auth identities are separate, lower layers and do exist. The live lab has also been upgraded to Wazuh 4.14.7 since the 4.14.5 submission baseline.
