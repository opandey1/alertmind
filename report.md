...
**The observed failure was model- and inference-configuration-dependent in this exploratory comparison, not intrinsic to LLM triage.** In the operational view, `llama3.1:8b` confirmed all 20 alerts as likely true positives and never returned "benign" (0/6 false positives identified). On the same corpus, the `gpt-5.5-2026-04-23` condition identified false positives without confidently misclassifying any, at 18/20 disposition vs 14/20. Under a **strict label-reduced evaluation view** (the tested code and detection-label classes verified removed), the two diverge sharply: llama3.1's genuine ATT&CK classification collapses to 1/14 attacks — it was largely copying the rule's label — while GPT-5.5 holds at 8/14 exact, 12/14 relaxed. The operational free-text grounding review began with a human-authored pass and received a provenance-recorded agent audit. Its current provisional results retain GPT-5.5 at 20/20 supported summaries and calibrated confidence, but reduce runnable query sets to 15/20; llama3.1 has 12/20 fully supported summaries, nine unsupported statements across eight partial summaries and 0/20 runnable query sets. The changed llama/GPT verdicts await explicit human adjudication (§9.5).

**Recommendation (two-part).** *`llama3.1:8b`:* do not deploy for false-positive triage (0/6). *GPT-5.5:* a promising candidate for further evaluation, but not deployment-approved from this experiment — tested once per view (a single stochastic sample), and no assisted-timing run was conducted with it. Its operational free-text outputs currently have 20/20 supported summaries and calibrated confidence but 15/20 runnable query sets; five query-validity corrections await explicit human adjudication (§9.5).
...
| Dimension | llama3.1:8b | gpt-5.5 | qwen3:8b |
|---|---:|---:|---:|
| Summary fully supported | 12/20 (8 partial) | **20/20** | 12/20 (8 partial) |
| Unsupported statements (total) | 9 | **0** | 8 |
| Investigation queries valid / runnable | **0/20** | **15/20** | **0/20** |
| Investigation queries relevant | 20/20 | 20/20 | 20/20 |
| Draft user message appropriate | 17/20 | **20/20** | 18/20 |
| Confidence calibrated | 13/20 | **20/20** | 18/20 |

All three worksheets began as human-authored reviews and then received the same evidence-led Codex second pass against the complete operational prompt. Each CSV and Markdown worksheet records the dimensions changed per alert. Qwen's corrected verdicts were explicitly approved by the human reviewer; the four changed llama rows (A07, A08, A16 and A20) and five changed GPT rows (A01, A04, A08, A16 and A17) await explicit human adjudication, so those two revised tallies are provisional. This agent-assisted audit is not an independent second human review.
...
| Free-text summary fully supported (grounding) | 12/20 (8 partial) | **20/20** |
| Investigation queries runnable (grounding) | **0/20** | **15/20** |
| Attacks classified benign (false negatives) | 0 | 1 (A18 — §11) |
| Valid JSON, matched operational+eval runs | **40/40** | **40/40** |

The grounding rows are from the separately identified **operational-view** review (§9.5). They are not strict-view grounding scores, and the llama grounding worksheet uses the earlier `20260713T115729Z` output sample rather than the matched operational comparison run. The current llama/GPT figures include the agent-assisted second-pass corrections and remain marked pending explicit human adjudication in the worksheets.
...
| **OpenAI Codex** | Evidence-led second-pass audit of the three operational grounding worksheets and documentation reconciliation | Every changed verdict dimension is recorded per alert. Qwen corrections were explicitly human-approved; changed llama3.1/GPT-5.5 rows remain marked pending human adjudication. The agent pass is not represented as an independent human review. |
