...
**Windows.** Process create (EID 1, EVID-WIN-001) and service creation (EID 7045 → rule 61138 → T1543.003, EVID-WIN-002) are verified in Wazuh. Seven custom Windows Sysmon rules (**100200–100206**) are deployed, each chaining off the relevant Wazuh built-in Sysmon base rule (EID 1 = 61603, EID 10 = 61612, EID 13 = 61615, EID 22 = 61624) and narrowing on `win.eventdata.*` fields: Office-spawns-shell (T1566/T1059), encoded PowerShell (T1059.001), LOLBins (T1218), LSASS access (T1003.001), PsExec service execution (T1021.002/T1569.002), Run-key persistence (T1547.001), and a DNS-tunnelling heuristic (T1048/T1071.004). **Three are verified firing:** encoded PowerShell (EVID-WIN-003), LOLBin execution (EVID-WIN-004), and Run-key persistence (EVID-WIN-005). Office-spawn, PsExec, and the DNS heuristic are scheduled for the Atomic phase (they need a real attack chain to trigger). Two tuning notes worth recording: **(a)** Run-key rule 100205 chains off Wazuh's built-in Run-key parent 92300 rather than the level-0 EID 13 base rule, and was then re-narrowed to exclude `CurrentVersion\RunNotification` (a shell notification key 92300 prefix-matches, which produced a duplicate FP in testing); **(b)** LSASS rule 100203 was tuned to dump-grade `GrantedAccess` masks after the untuned rule fired hundreds of times on benign query-only reads (AV/EdgeUpdate/svchost at `0x1000`) — it now stays quiet until a genuine dump and will be verified during the Atomic credential-dumping run.
