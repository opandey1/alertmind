...
The current suite contains **290 tests** covering provider request construction, schema/error metadata, runtime/formal-schema alignment, non-destructive audit reconstruction, explicit corpus-subset selection, redaction, strict-view label leakage, injection markers, boundary blocking, consent, ad hoc audit semantics, Streamlit state handling, and the offline RBAC contract checks that pin the committed identity, role, DLS, transport and rollback/revocation packages and the owner-reported drill evidence against drift. The local Dashboard broker-code inventory checks are not live broker authorization or TLS proof. This count includes ten builder guards for the [offline broker TLS candidate](siem/rbac/broker_tls/README.md) and twenty [operator-core tests](siem/rbac/broker_tls/OPERATOR.md) for byte checks and recovery decisions, plus eighteen native-reader methods (four Linux-only) plus ten root-controlled bootstrap and sixteen package/runtime and nineteen service-observation and seventeen process-observation tests (three Linux-only) and twelve fixed launch-file tests, not an installed startup guard or completed rollback implementation. The actual-client synthetic TLS fixture is a separate, opt-in check, not part of this CI count or live acceptance.
