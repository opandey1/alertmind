...
### 6.4c Post-v1 three-model extension — `qwen3:8b`

The two-model section above records the submitted-v1 milestone. After tagging v1, a second local 8B condition was pre-specified and run against the same frozen corpus, prompt and operational/strict views. The accepted Qwen pair is `20260830_054251_ollama_oper_baseline` / `20260830_070142_ollama_eval_baseline`, executed on commit `7ca2543` with `temperature=0.6`, `top_p=0.95`, seed 42, 4096-token budget and Ollama-compatible structured output. Its manifests pin normalized audit hashes, the model digest and effective request configuration.

| Metric (attacks n=14 unless noted) | llama3.1 strict | qwen3:8b op | qwen3:8b strict | GPT-5.5 strict |
|---|---:|---:|---:|---:|
| Technique exact / relaxed | 1/14 · 1/14 | 14/14 · 14/14 | 2/14 · 5/14 | 8/14 · 12/14 |
| Disposition (all 20) | 14/20 | 14/20 | 14/20 | 16/20 |
| Benign cleared / hedged / confidently wrong | 0/2/4 | 0/0/6 | 1/3/2 | 3/3/0 |
| Attack false negatives | 0 | 0 | 1 (A10) | 1 (A18) |
| Valid JSON across matched pair | 40/40 | 40/40 | — | 40/40 |

Operational grounding for Qwen is 12/20 fully supported summaries (eight partial), eight unsupported statements, 0/20 runnable and 20/20 relevant query sets, 18/20 appropriate drafts and 18/20 calibrated confidence. The same evidence-led second pass was applied to all three model worksheets, and one human explicitly approved every changed verdict on 2026-08-31. Query validity uses an all-or-nothing, target-aware standard: Wazuh/OpenSearch queries must use indexed fields such as `data.audit.*` / `data.win.*` and be runnable as written; rule-decoder names, prose, undefined pseudo-SQL and incomplete or unsafe shell fragments fail.

Qwen's strict median call latency was 168.21 s (59.83 min total), versus 60.37 s for llama3.1 and 10.66 s for hosted GPT-5.5 in this environment. The accepted Qwen pair is one sample, not a distribution; retained candidate/final outputs differed despite the recorded seed.
...
| 4 | Hallucination check too narrow (technique only, not the other 3 deliverables) | ✅ Completed for operational outputs | A human-authored six-dimension grounding pass over all 4 deliverables now has the same evidence-led Codex second pass for llama3.1, gpt-5.5 and qwen3:8b operational outputs (§9.5; sheets in `measurement/grounding/`). Current tallies: llama3.1 12/20 fully supported summaries and 0/20 runnable queries; gpt-5.5 20/20 and 15/20; qwen3:8b 12/20 and 0/20. Per-alert changed dimensions are recorded, and one human explicitly approved all changes on 2026-08-31. The agent pass is not an independent human reviewer; strict-view grounding and independent inter-rater review remain future work. |
...
- **Label leakage** inflates the operational technique number; the strict label-reduced view is the honest figure — **1/14 attacks exact for llama3.1, 2/14 for qwen3:8b and 8/14 for gpt-5.5** (the earlier llama 7/20 came from a still-leaky view). Lead with the strict number.
...
- **Small-model reliability** — an earlier llama3.1 run and retained Qwen pilots exposed malformed/truncated-output risk; the accepted operational and strict pairs are 40/40 valid for all three models, with Qwen using structured output plus the unchanged runtime validator.
- **Redaction is risk-reduction, not a guarantee** (§3.1).
- **Small n (20), single environment, one accepted pair per comparison model** — results are directional, not statistically powered. Three models were measured: `llama3.1:8b` (local, `temperature=0`; historical pair byte-identical), `qwen3:8b` (local, explicit sampling config and seed, but candidate/final outputs differed), and `gpt-5.5-2026-04-23` (hosted, pinned snapshot, stochastic vendor-default reasoning effort).
...
| Automate and independently repeat output grounding | The three human-approved operational worksheets have a human-authored pass plus a recorded agent-assisted evidence check. Automation, strict-view coverage and an independent second human reviewer remain future work; the agent pass does not establish inter-rater reliability. |
| ~~Add a third comparison model~~ | **Done post-v1:** qwen3:8b was run on the same frozen corpus, prompt and views, with automated scoring, provenance manifests and the same human-approved grounding protocol (§6.4c). |
...
Two numbers, and the distinction matters. In the matched *operational* view llama3.1 and qwen3:8b each score 14/14 on attacks — but that view shows the rule's own ATT&CK label, so it measures metadata consistency. Under the strict label-reduced view llama is **1/14 exact and relaxed**, Qwen is **2/14 exact / 5/14 relaxed**, and GPT-5.5 holds at **8/14 exact / 12/14 relaxed**. I report the strict number as the honest one; a first llama evaluation view still leaked (7/14) until I made it strict label-reduced and re-ran. It's why the two views exist.
...
We tested three conditions, including a second local 8B model. On the identical frozen corpus, qwen3:8b improved strict ATT&CK scoring over llama3.1 (2/14 exact / 5/14 relaxed versus 1/14 / 1/14) but remained weak on false positives (1/6 cleared strict, 0/6 operational) and was slower on this CPU. Hosted `gpt-5.5-2026-04-23` cleared 5/6 operationally (3/6 strict, 0 confidently wrong) and held at 8/14 exact / 12/14 relaxed. That is materially stronger in this sample, but each comparison is one accepted pair, configurations differ, and neither Qwen nor GPT was run through the assisted-timing pass. The result supports further evaluation, not deployment approval. The evaluation *method*—benign salt, strict label-reduced view, separated metrics and human-approved grounding—is what makes the comparison checkable.
...
**AI disclosure:** AI assistants (Claude and OpenAI Codex) were used as pair-programming, evidence-checking and review aids during development and analysis; one human made the final adjudications. Reported model outputs were produced by the stated providers: `ollama/llama3.1:8b` (`temperature=0`, historical pair byte-identical), `ollama/qwen3:8b` (explicit sampling/seed and structured-output configuration; accepted pair pinned by manifests), and `openai/gpt-5.5-2026-04-23` (pinned snapshot, vendor-default reasoning effort, stochastic single sample). Audit logs and manifests retain the effective configurations and served models; normalized hashes protect the accepted Qwen evidence. No determinism claim is generalized beyond the observed artifacts.
